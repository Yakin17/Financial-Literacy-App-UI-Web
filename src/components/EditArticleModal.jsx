...
import { utilisateurService } from '../services/apiService'; // Import utilisateurService directly
...
const EditArticleModal = ({ article, onClose, onSave }) => {
    ...
    const [auteurId, setAuteurId] = useState(article.auteurId);
    const [auteurs, setAuteurs] = useState([]);
    const [loading, setLoading] = useState(true);


    useEffect(() => {
        const fetchAuteurs = async () => { // Corrected async function declaration
            try {
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg shadow-xl w-full max-w-4xl">
                <div className="p-6">
                    <h2 className="text-2xl font-bold mb-4 text-gray-800">Modifier l'article</h2>
